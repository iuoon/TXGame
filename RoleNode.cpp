...
RoleNode *RoleNode::createWithLayer(HelloWorld *layer)
{
...
bool RoleNode::initWithLayer(HelloWorld *layer)
{
    bool bRet = false;
    do 
    {
...
void RoleNode::moveToward(const Point toTileCoord)
{
    ...
    if(!isHasBarrier)//起点终点两点之间无障碍，则不采用A＊避免在不必要的时候依然使用A*寻路
    {
        ...
        MoveTo *moveAction = MoveTo::create(0.1f*time, _layer->positionForTileCoord(toTileCoord));
...
void RoleNode::getBorderUponPlace()
{
    ...
    int time = sqrt(distanceX*distanceX+distanceY*distanceY);
    MoveTo *moveAction = MoveTo::create(0.1f*time, _layer->positionForTileCoord(endPosition));
    moveAction->setTag(1);
    this->runAction(moveAction);
}
//处理三点共线
...
void RoleNode::popStepAndAnimate()
{
    ...
    //组合动作和回调
    MoveTo *moveAction = MoveTo::create(0.1f*time, _layer->positionForTileCoord(step->getPosition()));
